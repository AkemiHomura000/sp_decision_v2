...
{ // 法向量法投影转平面
    ...
    {
        // 内存分配失败，进行错误处理
        ROS_INFO("Failed to allocate memory for pcl2cloud");
        return; // 或者采取其他合适的错误处理措施
    }
    sensor_msgs::PointCloud2 ROSPCL_output;
    pcl::fromROSMsg(*laserCloudMsg, *pcl2cloud);
    pcl::PointCloud<pcl::PointXYZ>::Ptr pcl2cloud_out(new pcl::PointCloud<pcl::PointXYZ>);
    ...
    {
        // 内存分配失败，进行错误处理
        ROS_INFO("Failed to allocate memory for pcl2cloud_out");
        return; // 或者采取其他合适的错误处理措施
    }
    ...
    else
    {
        ...
        {
            // 内存分配失败，进行错误处理
            ROS_INFO("Failed to allocate memory for tree");
            return; // 或者采取其他合适的错误处理措施
        }
        ...
        {
            // 内存分配失败，进行错误处理
            ROS_INFO("Failed to allocate memory for cloud_normals");
            return; // 或者采取其他合适的错误处理措施
        }
